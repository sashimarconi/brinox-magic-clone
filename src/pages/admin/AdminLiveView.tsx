...
const AdminLiveView = () => {
  ...
  const fetchData = useCallback(async () => {
    const now = new Date();
    const ninetySecAgo = new Date(now.getTime() - 90 * 1000).toISOString();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString();

    const [sessionsRes, ordersRes, eventsRes, todaySessionsRes] = await Promise.all([
      supabase.from("visitor_sessions").select("session_id, page_url, last_seen_at, city, region, country, latitude, longitude").gte("last_seen_at", ninetySecAgo),
